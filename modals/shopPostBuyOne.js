...
module.exports = {
	data: {name:"shopPostBuyOne"},
	async execute(interaction, sequelize, models, btndata) {
		try {
			await interaction.deferReply({ephemeral:true})
			let name=await interaction.fields.getTextInputValue("character-name").trim()
			...
			if(character.balance-metadata.price<0){
				return interaction.editReply({content:`Not enough coin to buy ${metadata.item}, missing ${toGold(metadata.price-character.balance)}.`, ephemeral:true});
			}
			let warning=""
			if(metadata.playercreated){
				...
				interaction.message.edit({embeds:[embed], components:[btns]})
				try{
			...
			return interaction.editReply({content:`${character.charname} bought one ${metadata.item} for ${toGold(metadata.price)}. (remainining balance: ${toGold(character.balance)})`, ephemeral:true})
