const { SlashCommandBuilder, ActionRowBuilder, TextInputBuilder, EmbedBuilder, TextInputStyle,  PermissionFlagsBits, ModalBuilder, StringSelectMenuBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
			...
			if(metadata.playercreated){
				...
				try{
					await interaction.client.users.fetch(target.owner)
						.then(async(user)=>{await user.send(`${character.charname} bought ${metadata.item} from ${target.charname} for ${toGold(metadata.price)}. New balance: ${toGold(target.balance)}`)})
				}catch(e){
					console.error(e)
				}
